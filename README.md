...
We'll use [NetLogo 6.0.4](https://ccl.northwestern.edu/netlogo/download.shtml) for the entire workshop. Earlier versions should work for the workshop as well, but no promises.
...
1. Download Netlogo 6.0.4 from [https://ccl.northwestern.edu/netlogo/download.shtml](). Note that filling in your information to get to the actual download page is optional.
...
4. Out of the install process you'll typically end up with a folder called something like "NetLogo 6.0.4". Put this wherever you would like the NetLogo program to be kept. If you're not sure you want to keep using it _or_ you want to be able to find it easily to play with then the Desktop is fine (easy to find to delete in the first case and easy to find and use in the second). _Do not separate out the contents of the folder._
5. Open the folder and inside should be an application called "NetLogo 6.0.4". Run this and as long as it starts without complaint you should be good to go.
...
2. **Look, look, and look again.** Even simple models can easily have subtle yet meaningful interactions. It is really important to know your model from the code to the output and from the output to the code. To do this you'll need to look at your code and your output _a lot_, especially if you think you have something that is non-trivial.
...
**Loading:** To load the model, from menu choose `File` > `Models Library`. A window should pop up with a list of folders and potential models to be chosen down the left side and a space for summary information on the right side. Navigate the folders on the left with  `Sample Models` > `Biology` > `Rabbits Grass Weeds`. You can also use the search bar located at the bottom of the Navigation Pane. 

While the Info pane has a description about what the model is meant to illustrate, one of the nice things about agent-based models is that much can be figured out just be running the model. _Most_ models in NetLogo will have a two step process for running the model and we'll follow this right now:

1. **setup**.  There will likely be a button called "setup" or similar. Clicking this button will load whatever values the model is intended to run under as a default and usually changes the central visualization pane from being empty to having initial content. Click "setup" now.
...
Lastly, open the Code panel. Just leave everything how it is (we'll get to playing with code shortly) and simply note that there really isn't a lot of code driving this simulation despite its ability to simulate hundreds of rabbits and 41 x 41 = 1,681 patches
...
* There are two broad classes of agents to consider when building a model in NetLogo: turtles and patches. Turtles move around on a grid of patches and the patches remain locked in place while any turtles move across them.
...
**Loading:** To load the model, from menu choose `File` > `Models Library`. A window should pop up with a list of folders and potential models to be chosen down the left side and a space for summary information on the right side. Navigate the folders on the left with  `Sample Models` > `Sociology` > `Wealth Distribution`. You can also use the search bar located at the bottom of the Navigation Pane. 
...
4. Note that the slider below the map-view can be used to zoom in or out. Also note that there is a "watch" button that will highlight the agent on the main map and follow it as the simulation progresses. 
...
2. If you bump into an object and you are carrying an object then put that object down, otherwise pick up the object you bumped into.

**Loading:** To load the model, from menu choose `File` > `Models Library`. A window should pop up with a list of folders and potential models to be chosen down the left side and a space for summary information on the right side.  Navigate the folders on the left with  `Sample Models` > `Biology` > `Termites`. You can also use the search bar located at the bottom of the Navigation Pane. 

Get a feel for this simulation by running it a few times and looking at the Info pane. Change the slider values both before the simulation starts and after.

> Come to an agreement with your group regarding what the rules of behaviour are for the termites simply by watching the simulation.  Write these rules down.
> 
...
**Loading:** To load the model, from menu choose `File` > `Models Library`. A window should pop up with a list of folders and potential models to be chosen down the left side and a space for summary information on the right side. Navigate the folders on the left with  `Sample Models` > `Computer Science` > `Cellular Automata` > `Life`. You can also use the search bar located at the bottom of the Navigation Pane. 
...
> 
> Right click on the visualization space and choose "Edit". Change the size of the space and run _Life_ again. Reset to the original size of 50x50 and then turn off "World wraps horizontally" and "World wraps vertically". Run _Life_ again. Confer with your group and list the consequences of these changes.
> 
...
**Loading:** To load the model, from menu choose `File` > `Models Library`. A window should pop up with a list of folders and potential models to be chosen down the left side and a space for summary information on the right side. Navigate the folders on the left with  `Sample Models` > `Computer Science` > `Cellular Automata` > `Life Turtle-Based`. You can also use the search bar located at the bottom of the Navigation Pane. 
...
**Loading:** To load the model, from menu choose `File` > `Models Library`. A window should pop up with a list of folders and potential models to be chosen down the left side and a space for summary information on the right side. Navigate the folders on the left with  `Sample Models` > `Social Science` > `Segregation`. You can also use the search bar located at the bottom of the Navigation Pane. 
...
To get these kinds of confirmation we typically need a lot of single runs and that will take a long time and a lot of clicking.  NetLogo has an answer for this: BehaviorSpace.
...
>
> The runs aren't in order, why is this?
>  
...
4. This will take a fair bit longer than the first set because we're doing 550 runs in total rather than 100. Further, some of these runs won't settle as they did in the first case meaning that they'll go to the cut off. You can speed things up by unchecking the boxes for Updating view and updating the plots and monitors located at the top of the Interface. You can also move the slider to "faster".
5. When complete, open the files in a program that can handle comma separated value files (e.g. Excel) and examine the output.
